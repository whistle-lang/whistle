...
pub fn parse_none_lit(parser: &mut Parser) -> Result<Literal, ParserError> {
  parser.eat_tok(Token::Keyword(Keyword::None))?;
  Ok(Literal::None)
}

pub fn parse_lit(parser: &mut Parser, literal: Literal) -> Result<Operand, ParserError> {
  Ok(Operand::Literal(match literal {
